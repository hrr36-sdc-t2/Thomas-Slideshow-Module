...
const PhotoGrid = ({images, selectActivePhoto, scrollToActive, setCaption, copyUrl, hideFilmstrip}) => (
  <div className="img-grid container-fluid">
    ...
    <div className="photogrid row h-100">
      {/* {Main image} */}
      <div className="col"><img id="image1" src={`http://${process.env.DB_URI}:2999/images/${images[0].imgPath}`} onClick={() => {selectActivePhoto(); setCaption()}}/></div>
      ...
      <div className="col-sm-4 col-lg-3 border-left-0 d-none d-sm-block">
        <div className="row h-50 border-top-0 border-bottom-0 border-left-0">
          <img id="image2" src={`http://${process.env.DB_URI}:2999/images/${images[1].imgPath}`} onClick={() => {selectActivePhoto(); setCaption()}}/>
        </div>
        <div className="row h-50 border-bottom-0 border-left-0">
          <img id="image3" src={`http://${process.env.DB_URI}:2999/images/${images[2].imgPath}`} onClick={() => {selectActivePhoto(); setCaption()}}/>
        </div>
      </div>
      {/* {Subimages for sizes lg and up} */}
      <div className="col-lg-3 border-0 d-none d-lg-block">
        <div className="row h-50 border-bottom-0">
          <img id="image4" src={`http://${process.env.DB_URI}:2999/images/${images[3].imgPath}`} onClick={() => {selectActivePhoto(); setCaption()}}/>
        </div>
        <div className="row h-50">
          <img id="image5" src={`http://${process.env.DB_URI}:2999/images/${images[4].imgPath}`} onClick={() => {selectActivePhoto(); setCaption()}}/>
        </div>
      </div>
    </div>
            ...
            <div id="carousel-custom" className="carousel slide" data-ride="carousel" data-interval="false">
              ...
              <div className="carousel-inner" role="listbox">
                <div className="image-arrow-wrapper">
                  <div className="carousel-item active">
                    <img className="d-block" src={`http://${process.env.DB_URI}:2999/images/${images[0].imgPath}`} alt={images[0].description}/>
                  </div>
              ...
              <div className="caption-filmstrip-wrapper">
                  <div className="caption-hide-wrapper">
                    <div className="img-caption">{1}/{images.length}: {images[0].description}</div>
                  ...
                  <div className="filmstrip">
                    <ol className="carousel-indicators">
                      <li data-target="#carousel-custom" data-slide-to="0" className="thumbnail active">
                        <img src={`http://${process.env.DB_URI}:2999/images/${images[0].imgPath}`} alt={images[0].description} className="img-responsive" onClick={() => setCaption()}/>
